...
class KernelLogisticRegression:
    ...
    def _stable_sigmoid(self, z):
        z = np.clip(z, -500, 500)  # limit range to avoid overflow
        return 1 / (1 + np.exp(-z))
...
class KernelSVM:
    def __init__(self, lambda_reg=0.01, degree=3, coef0=1, epochs=15, eta=0.1):
        self.lambda_reg = lambda_reg
        self.degree = degree
        self.coef0 = coef0
        self.epochs = epochs
    ...
    def fit(self, X, y):
        ...
        self.X_train = X.copy()     
        self.y_train = y.copy()
        self.K_train = poly_kernel(X, X, self.degree, self.coef0)
        self.K_train = poly_kernel(X, X, self.degree, self.coef0)
        self.alpha = np.zeros(n)
        self.b = 0.0
        self.y_train = y.copy()
